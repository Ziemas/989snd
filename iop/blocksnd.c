...
/* 000097bc 00009a98 */ UInt32 snd_PlaySFXGrain(/* 0x0(sp) */ SFXBlock2Ptr block, /* 0x4(sp) */ SInt32 sindex, /* 0x8(sp) */ SInt32 gindex) {
    ...
    hand = snd_GetFreeBlockSoundHandler(NULL, 0, false);
    if (hand == NULL) {
        snd_UnlockMasterTick();
        return 0;
    }
...
/* 0000b328 0000b45c */ SInt32 snd_SFX_GRAIN_TYPE_LOOP_CONTINUE(/* 0x0(sp) */ BlockSoundHandlerPtr handler, /* 0x4(sp) */ SFX2Ptr sfx, /* 0x8(sp) */ SFXGrain2Ptr grain) {
    ...
    for (i = handler->NextGrain + 1; i < sfx->NumGrains && !found; i++) {
        if (sfx->FirstGrain[i].OpcodeData.MicroOp.Type == GRAIN_LOOP_END) {
            handler->NextGrain = i;
            found = 1;
        }
    }
...
/* 0000b8c8 0000b9b8 */ SInt32 snd_SFX_GRAIN_TYPE_PB(/* 0x0(sp) */ BlockSoundHandlerPtr handler, /* 0x4(sp) */ SFX2Ptr sfx, /* 0x8(sp) */ SFXGrain2Ptr grain) {
    /* -0x10(sp) */ SInt32 work32;
    if (GRAIN_ARG(0) >= 0) {
        work32 = 0x7fff * GRAIN_ARG(0) / 127;
    } else {
        work32 = -0x8000 * GRAIN_ARG(0) / -128;
    }
...
/* 0000c284 0000c448 */ SInt32 snd_SFX_GRAIN_TYPE_GOTO_RANDOM_MARKER(/* 0x0(sp) */ BlockSoundHandlerPtr handler, /* 0x4(sp) */ SFX2Ptr sfx, /* 0x8(sp) */ SFXGrain2Ptr grain) {
    ...
    work32 = GRAIN_ARG(1) - GRAIN_ARG(0) + 1;
    work32 = snd_RandomUInt16() % work32 + GRAIN_ARG(0);
...
/* 0000c934 0000ca64 */ SInt32 snd_SFX_GRAIN_TYPE_COPY_REGISTER(/* 0x0(sp) */ BlockSoundHandlerPtr handler, /* 0x4(sp) */ SFX2Ptr sfx, /* 0x8(sp) */ SFXGrain2Ptr grain) {
    /* -0x10(sp) */ SInt8 work;
    if (GRAIN_ARG(0) < 0) {
        work = gBlockGlobalReg[-GRAIN_ARG(0) - 1];
    } else {
        work = handler->Registers[GRAIN_ARG(0)];
    }

    if (GRAIN_ARG(1) < 0) {
        gBlockGlobalReg[-GRAIN_ARG(1) - 1] = work;
    } else {
        handler->Registers[GRAIN_ARG(1)] = work;
    }
...
/* 0000cb1c 0000cec8 */ SInt32 snd_SFX_GRAIN_TYPE_BRANCH(/* 0x0(sp) */ BlockSoundHandlerPtr handler, /* 0x4(sp) */ SFX2Ptr sfx, /* 0x8(sp) */ SFXGrain2Ptr grain) {
    ...
    /* -0x14(sp) */ BlockSoundHandlerPtr weakest = NULL;
    /* -0x10(sp) */ PlaySoundParams *psp;
    psp = handler->block->GrainData + (grain->OpcodeData.Opcode & 0xFFFFFF);
    if (psp->sound_id < 0) {
        block = NULL;
        index = snd_FindSoundByName(0, psp->snd_name, &block);
    ...
    if ((new_sfx->Flags & SFX_INSTLIMIT) != 0) {
        if (!snd_CheckInstanceLimit(new_sfx, handler->App_Vol, false, &weakest)) {
            return -1;
        }

        if (weakest && weakest != handler) {
            snd_StopHandlerPtr(weakest, 1, 0, 1);
        }
    }

    new_sfx->Flags |= SFX_BRANCH_TARGET;
    if (new_sfx->NumGrains == 0) {
        return -1;
    }
...
/* 0000d548 0000d740 */ SInt32 snd_SFX_GRAIN_TYPE_STOPCHILDSOUND(/* 0x0(sp) */ BlockSoundHandlerPtr handler, /* 0x4(sp) */ SFX2Ptr sfx, /* 0x8(sp) */ SFXGrain2Ptr grain) {
    ...
    while (snd_KillChildrenWithSound(&handler->SH, &block->FirstSound[index]))
        ;
